...
(define (kahua-application-environment)
  (let ((sandbox (make-sandbox-module))
    ...
    (for-each
      (lambda (e)
        (eval `(define-in-module ,sandbox ,(car e) ,(cdr e)) cm))
...
(define-syntax with-app
  (syntax-rules ()
    ((_ form) (reset (eval 'form kahua-app-server)))))

(define main-proc
  (make-parameter (lambda _ (error "Not initialized!"))))
...
(define (database-name)
  (kahua-dbpath (or (primary-database-name) (kahua-default-database-name))))
...
(define (handle-request header body reply-cont selector)
  (with-sigmask SIG_BLOCK *TERMINATION-SIGNALS*
    (lambda ()
      (let1 db (current-db)
        (unless (kahua-db-ping db) (kahua-db-reopen db))
        (let1 do-reply
          (guard (e (else
              ...
              (lambda (db)
                ...
                (begin0
                  (kahua-default-handler
                    header body reply-cont default-handler
  ...
  (let ((sock (make-server-socket sockaddr :reuse-addr? #t :backlog SOMAXCONN))
    ...
    (define (orphan-handler in flag)
      (when (eof-object? (read-byte in))
        (display "[server]: parent, probably spvr, has stopped, so kill myself.\n" (current-error-port))
  ...
  (let-args (cdr args)
    ...
    (let* ((worker-name (car (string-split (sys-basename (car mods)) #\.))) 
           ...
           (cleanup (lambda ()
                      ...
                      (and-let* ((db (current-db)) ((active? db)))
                        (kahua-db-close db #f)))))
      (begin0
        (call/cc
          (lambda (bye)
