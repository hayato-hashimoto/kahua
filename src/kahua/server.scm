...
(define kahua-bridge-name (make-parameter #f)) ;; dummy

;; KAHUA-SERVER-URI
;;   A parameter that holds the server URI (scheme://server:port).
;;   The cgi-bridge tells this info to the app server.
(define kahua-server-uri (make-parameter #f)) ;; dummy

(define kahua-worker-uri (make-parameter #f))

(define (kahua-base-uri)
  (or 
    (and (kahua-bridge-name) (build-path (kahua-bridge-name) 'up 'up))
    (kahua-server-uri)
    ""))
...
(define (kahua-self-uri . paths)
  (path-info->abs-path
   (simplify-path-info
    (cond
      ((worker-uri) => (lambda (w) (append! (string-split w #\/) paths)))
      (else (append! (string-split (kahua-bridge-name) #\/)
    ...
    (receive (state-id cont-id) (get-gsid-from-header header)
      (let* ((state-id  (or state-id (session-state-register)))
        ...
        (parameterize ((kahua-bridge-name
                         (assoc-ref-car header "x-kahua-bridge" #f))
                       (kahua-server-uri
                         (assoc-ref-car header "x-kahua-server-uri" #f))
                       (kahua-worker-uri
                         (assoc-ref-car header "x-kahua-worker-uri" #f)))
          (if (assoc-ref-car header "x-kahua-eval" #f)
            (receive (headers result) (run-eval state)
              (lambda () (reply-cont headers result)))
            (let*-values (((proc permanent?)
                            (if cont-id
              ...
              (let1 session-domain (kahua-session-domain-info)
                (lambda ()
                  (reply-cont
                    (kahua-merge-headers
                      (acons "x-kahua-session-domain" session-domain (alist-delete "x-kahua-metavariables" header))
                      (assoc-ref-car context "extra-headers" '())
                      (hash-table-map (assoc-ref-car context "x-kahua-headers" '()) list))
                    stree))))))))))
...
(define (kahua-render-proc nodes context)
  (let* ((expanded
    ...
    (receive (expanded context)
      (prepare
         expanded
         (list* `("x-kahua-keep-client-context" . #f)
                `("x-kahua-expanded-node" . (,expanded))
                context)
         values)
    ...
    `(begin
       ...
       (define-method ,name ,method-args
         (let1 rules (hash-table-get (ref ,name 'rules) ',specs)
           (,apply-rule
             rules
             (append (list ,@(map identity lambda-args)) ,rarg)))))))
